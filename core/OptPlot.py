...
OptPlot is a lightweight plotting widget for wxPython for displaying pyFSRS measurement results.
...
class OptPlot(wx.Panel):
    """The OptPlot class is derived from wxPanel and provides a lightweight plotting interface
    ...
    def initVariables(self):
        ...
        # list of line colors; the color is cycled through the array for each new line or point
        self.linescolor = [wx.Colour(255, 0, 0), wx.Colour(255, 165, 0), wx.Colour(255, 255, 0),
                           wx.Colour(50,205,50),wx.Colour(0, 255, 255), wx.Colour(0, 0, 255), wx.Colour(255,0,255)]
    ...
    # DATA HANDLING
    def addImage(self, x, y, z,domain0=[[-10.0,10.0],[-10.0,10.0]]):
        """Add an image plot.
        ...
        self.data_extent.append((domain0[0,0], domain0[0,1], domain0[1,0], domain0[1,1]))
        self.Refresh()
        return id

    def addContour(self, x, y, z,domain0=[[-10.0,10.0],[-10.0,10.0]]):
        """Add contour plot.
    ...
    def addLine(self, x, y,domain0=[[-10.0,10.0],[-10.0,10.0]]):
    ...
    def addScatter(self, x, y,domain0=[[-10.0,10.0],[-10.0,10.0]]):
        """Add a scatter plot.
    ...
    def setLine(self, id, x, y,domain0=[[-10.0,10.0],[-10.0,10.0]]):
        ...
        if isinstance(x, list) or isinstance(x, np.ndarray):
            ...
            nd = np.array([[float(x), float(y)], ])
        self.data[id] = nd
        self.data_extent[0] = (domain0[0,0], domain0[0,1], domain0[1,0], domain0[1,1])
        #self.data_extent.append((np.nanmin(x), np.nanmax(x), np.nanmin(y), np.nanmax(y)))
        self.Refresh()

    def setScatter(self, id, x, y,domain0=[[-10.0,10.0],[-10.0,10.0]]):
        """Same as `setLine`."""
        self.setLine(id, x, y,domain0)
...
class DualPlotFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.upperPlotCanvas = OptPlot(self.splitter)
        self.lowerPlotCanvas = OptPlot(self.splitter)
